...
{
    public class ConsultaVendasController : ApiController
    {
        public HttpResponseMessage Get(string token, int colecao = 0, int campo = 0, int orderBy = 0, int pageSize = 0, int pageNumber = 0)
        {
            ...
            using (painel_taxservices_dbContext _dbAtos = new painel_taxservices_dbContext())
            {
                ...
                try
                {
                    ...
                    {
                        Retorno dados = GatewayConsultaVendas.Get(token, colecao, campo, orderBy, pageSize, pageNumber, queryString, _dbAtos);//, _dbAlpha);
